...
class InnerBridge(private val context: Context, private val webView: WebView, private val name: String = "JSBridge") {
    ...
    private val pendingPromises = mutableMapOf<Long, Pair<CallResolver<*>, KClass<*>>>()
    ...
    fun finishPromise(promiseBinding: Long, answerAsString: String) {
        ...
        val callResolver = promisePair.first as CallResolver<Any?>
        val kClass = promisePair.second

        executor.execute {
            when {
                answer.hasError -> callResolver.reject(Exception("${answer.error?.message} ${answer.error?.stackTrace}"))
                kClass == Unit::class -> callResolver.resolve(Unit)
                else -> callResolver.resolve(gson.fromJson(answer.value, kClass.java))
            }
        }
    }
    ...
    fun <R> callJSFunctionWithPromise(jsFunctionWithPromise: JSFunctionWithPromise<R>): CallResolver<R> {
        val callResolver = CallResolver<R>()
        val pendingPromiseID = currentPendingPromiseID.getAndIncrement()
        pendingPromises[pendingPromiseID] = Pair(callResolver, jsFunctionWithPromise.kClass)
        executeJavaScript("${name}.executeFunctionWithPromiseBinding(${jsFunctionWithPromise.functionBinding},${pendingPromiseID})")
        return callResolver
    }

    fun <A, R> callJSFunctionWithPromise(jsFunctionWithPromise: JSFunctionWithPromiseAndArg<A, R>, arg: A): CallResolver<R> {
        val callResolver = CallResolver<R>()
        val pendingPromiseID = currentPendingPromiseID.getAndIncrement()
        pendingPromises[pendingPromiseID] = Pair(callResolver, jsFunctionWithPromise.kClass)
    ...
    private fun handlePromise(callResolver: CallResolver<*>, call: Call) {
    ...
    private fun createFunctionJS(jsInterfaceData: JSInterfaceData, kFunction: KFunction<*>): String {
        ...
        val function = when (nativeCall.value) {
            ...
            CallType.WEB_CALL -> "nativeCallWebPromise"
    ...
    private fun validateFunction(kFunction: KFunction<*>) {
        val nativeCall = kFunction.annotations.filterIsInstance<NativeCall>().first()
        if (nativeCall.value == CallType.FULL_PROMISE && kFunction.returnType.kClass() != CallResolver::class) {
