...
startApp(() => {
    ...
    android.registerFunctionWithPromise(() => {
        return new Promise<string>((resolve) => {resolve("Hello from java script "))})
    ...
    document.getElementById("fetchDataButton").addEventListener("click", function () {
            android.fetchDataViaNative().then(function (response) {
                appendText("Response from Native: " + response);
            }).catch(function (err) {
                console.error("Native call failed: " + err.toString());
                appendText("Error: " + err.toString());
            });
        });
})
